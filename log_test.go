...
func TestEmptyLevelFieldName(t *testing.T) {
	...
	t.Run(
		"empty setting", func(t *testing.T) {
			out := &bytes.Buffer{}
			log := New(out)
			log.Info().
				Str("foo", "bar").
...
func TestWith(t *testing.T) {
	...
	log.Log().Msg("")
	if got, want := decodeIfBinaryToString(out.Bytes()), `{"caller":"`+caller+`","string":"foo","stringer":"127.0.0.1","stringer_nil":null,"bytes":"bar","hex":"12ef","json":{"some":"json"},"bool":true,"int":1,"int8":2,"int16":3,"int32":4,"int64":5,"uint":6,"uint8":7,"uint16":8,"uint32":9,"uint64":10,"float32":11.101,"float64":12.30303,"time":"0001-01-01T00:00:00Z"}`+"\n"; got != want {
		t.Errorf("invalid log output:\ngot:  %v\nwant: %v", got, want)
	}
...
func TestLevel(t *testing.T) {
	...
	t.Run(
		"NoLevel/Disabled", func(t *testing.T) {
			out := &bytes.Buffer{}
			log := New(out).Level(Disabled)
			log.Log().Msg("test")
			if got, want := decodeIfBinaryToString(out.Bytes()), ""; got != want {
	...
	t.Run(
		"NoLevel/Panic", func(t *testing.T) {
			out := &bytes.Buffer{}
	...
	t.Run(
		"NoLevel/WithLevel", func(t *testing.T) {
			out := &bytes.Buffer{}
			log := New(out).Level(InfoLevel)
...
func TestContextTimestamp(t *testing.T) {
	...
	if got, want := decodeIfBinaryToString(out.Bytes()), `{"time":"2001-02-03T04:05:06Z","foo":"bar","message":"hello world"}`+"\n"; got != want {
		t.Errorf("invalid log output:\ngot:  %v\nwant: %v", got, want)
	}
}
